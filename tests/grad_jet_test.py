...
def test_grad_sol_mlp():
  ...
  def sol_M1(M1):
    # wrapper for expected arguments of sol
    f_dynamics = lambda x,t : mlp(M1,M2,x)
    return sol_recursive(f_dynamics,x,1.)

  # print(sol_M1(M1))
  # print((lambda m1: np.sum(sol_M1(m1)[0]))(M1))
  print(grad(lambda m1: np.sum(sol_M1(m1)[1][4]))(M1))
